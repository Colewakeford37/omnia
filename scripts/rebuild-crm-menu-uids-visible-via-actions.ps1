...
foreach ($def in $defs) {
  ...
  $baseSafe = $uid.Replace('-', '_')
  $blockUid = ($baseSafe + "_block_" + $suffix)
  $tableUid = ($baseSafe + "_table_" + $suffix)
  ...
  foreach ($col in $def.columns) {
    $colUid = ($baseSafe + "_col_" + $col + "_" + $suffix)
  ...
  $results += [pscustomobject]@{
    ...
    columnInsertOkCount = ($colResults | Where-Object { $_.ok }).Count
...
[pscustomobject]@{ summary = $summary; results = $results } | ConvertTo-Json -Depth 30 | Set-Content -Path "scripts/rebuild-crm-menu-uids-visible-via-actions-output.json"
